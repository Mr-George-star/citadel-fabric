...
public class LightningBoltData {
        ...
        for (int i = 0; i < this.count; i++) {
            ...
            while (!drawQueue.isEmpty()) {
                ...
                if (progress >= 1) {
                    ...
                    float maxDiff = this.renderInfo.spreadFactor * segmentDiffScale * totalDistance * this.renderInfo.randomFunction.getRandom(random);
    ...
    public interface RandomFunction {
        ...
        RandomFunction GAUSSIAN = random -> (float) random.nextGaussian();
    ...
    public interface SegmentSpreader {
        ...
        static SegmentSpreader memory(float memoryFactor) {
            return (perpendicularDist, randVec, maxDiff, spreadScale, progress) -> {
                ...
                Vec3d current = randVec.multiply(nextDiff);
                if (progress > 0.5F) {
                    // begin to come back to the center after we pass the halfway mark
                    current = current.add(perpendicularDist.multiply(-1 * (1 - spreadScale)));
                }
                return perpendicularDist.add(current);
            };
        }
    ...
    public interface SpawnFunction {
        ...
        static SpawnFunction delay(float delay) {
            return (random) -> Pair.of(delay, delay);
        }
        ...
        static SpawnFunction noise(float delay, float noise) {
            return (random) -> Pair.of(delay - noise, delay + noise);
        }
    ...
    public interface FadeFunction {
        ...
        /** Reader bolts with a segment-by-segment 'fade' in and out, with a specified fade duration (applied to start and finish). */
